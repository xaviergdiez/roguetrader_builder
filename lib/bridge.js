...
// May this account write these fields of this ship?
//
// The GM is checked first and needs no station, which is the normal case for
// one running NPC ships. A non-member is refused outright — being signed in is
// not being at the table.
export function authorizePatch(dynasty, uid, fields) {
  const gm = isGm(dynasty, uid);
  if (!gm && !memberOf(dynasty, uid)) {
    return { ok: false, reason: "not_a_member", denied: fields || [], isGm: false, role: "" };
  }
  const role = roleOf(dynasty, uid);
  const denied = deniedWrites(role, fields, { isGm: gm });
  return {
    // All or nothing: a half-applied order leaves the ship in a state no
    // player chose.
    ok: denied.length === 0,
    reason: denied.length ? "forbidden_fields" : null,
    denied,
    isGm: gm,
    role
  };
...
export function redactShip(ship, { isGm: gm = false } = {}) {
  ...
  return {
    ...
    fleet: (ship.fleet || []).map((enemy) => {
      if (enemy && enemy.scanned) return enemy;
      const out = {};
