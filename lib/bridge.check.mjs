...

{
  ...
  // weapons/armour are clamped the same as any other list field
  const geared = cleanCard({
    weapons: ['good hellgun', 'mono-sword'],
    armour: Array.from({ length: 200 }, (_, i) => 'armour' + i)
  });
  assert.deepEqual(geared.weapons, ['good hellgun', 'mono-sword']);
  assert.equal(geared.armour.length, CARD_MAX_ENTRIES);
  assert.deepEqual(cleanCard({}).weapons, [], 'missing weapons/armour clean to empty lists, not undefined');
}
...
/* ==============================================================================
   Ground combat: wounds mirror and event authorization. */

const { mirrorPlayerWounds, authorizeGroundEvent } = await import('./bridge.js');

/* ---- newShip's ground combat shape ---- */

{
  const ship = newShip('AB3K9P');
  assert.deepEqual(ship.combat.playerVitals, {});
  assert.deepEqual(ship.combat.ground, { npcs: [] });
}

/* ---- mirroring wounds on join ---- */

{
  const ship = newShip('AB3K9P');
  const next = mirrorPlayerWounds(ship, 'char1', { max: 13, damage: 5, critSoFar: 0 });
  assert.equal(next.combat.playerVitals.char1.max, 13);
  assert.equal(next.combat.playerVitals.char1.damage, 5);
  assert.equal(next.rev, ship.rev + 1, 'mirroring bumps rev, same as any other write');

  // Damage is clamped to max even on the way in, same as wounds.js's own rule.
  const clamped = mirrorPlayerWounds(ship, 'char1', { max: 10, damage: 99 });
  assert.equal(clamped.combat.playerVitals.char1.damage, 10);
}

{
  // No wounds payload: the ship is returned untouched, so a re-join to
  // switch station does not wipe an in-progress encounter.
  const ship = newShip('AB3K9P');
  const seeded = mirrorPlayerWounds(ship, 'char1', { max: 13, damage: 5 });
  const rejoined = mirrorPlayerWounds(seeded, 'char1', null);
  assert.equal(rejoined, seeded, 'no payload leaves the ship untouched');
}

{
  // A bad charId is refused rather than writing a garbage key.
  const ship = newShip('AB3K9P');
  const next = mirrorPlayerWounds(ship, 'not a valid id!', { max: 10 });
  assert.equal(next, ship);
}

/* ---- ground event authorization delegates the dynasty-shaped facts ---- */

{
  const dynasty = newDynasty({ code: 'AB3K9P', name: 'Test', ownerUid: 'gm1' });
  const { dynasty: joined } = joinDynasty(dynasty, {
    uid: 'p1', charId: 'char1', name: 'Sarge', role: 'ordnance'
  });

  // The GM may fire anything.
  assert.equal(authorizeGroundEvent(joined, 'gm1', { id: 'npc_attack', charId: 'char1' }).ok, true);

  // The player may act on their own character...
  assert.equal(authorizeGroundEvent(joined, 'p1', { id: 'player_damage', charId: 'char1' }).ok, true);
  // ...never someone else's...
  assert.equal(
    authorizeGroundEvent(joined, 'p1', { id: 'player_damage', charId: 'someone-else' }).reason,
    'not_your_character'
  );
  // ...and never a GM-only event, even against themselves.
  assert.equal(
    authorizeGroundEvent(joined, 'p1', { id: 'npc_attack', charId: 'char1' }).reason,
    'gm_only'
  );
  // A stranger to the table is refused outright.
  assert.equal(
    authorizeGroundEvent(joined, 'nobody', { id: 'player_damage', charId: 'char1' }).reason,
    'not_a_member'
  );
}

console.log('bridge: ground combat checks passed');
